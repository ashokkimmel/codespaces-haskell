...
{-# LANGUAGE Safe #-}
module Dimensions.TypeMisc (type (<>),Maybe',FoundZero,IfThenElse) where
import GHC.TypeLits qualified as TL
import GHC.TypeLits (Symbol)
import GHC.TypeError ( TypeError, ErrorMessage(Text) )
type FoundZero :: k
type FoundZero = TypeError ('Text "There should not be a zero exponent in a dimension, please report this as a bug.")
